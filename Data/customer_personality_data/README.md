...
- **Regression target: `Income`.** Rows with missing `Income` are dropped in `regression.ipynb` (imputing the target itself would be the wrong move) — 2,213 of 2,237 cleaned rows remain (24 dropped). By far the strongest bivariate regression signal found across all three datasets explored (17 of 25 candidate features clear the 0.1 relevance threshold, several exceeding 0.5) — see `REGRESSION_NOTES.md`'s Discussion for the full contrast against `balance` and `Rating`'s "no dominant predictor" finding in the other two datasets. Candidates include `Response` (scored like any other feature, not excluded by convention) — it clears the bivariate threshold at 0.133 but is demoted to near-zero by RF and MI once the spend/purchase-channel cluster is available, so it isn't a chosen feature.
